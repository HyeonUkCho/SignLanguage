...
gboolean send_data(char *message)
{
	...
	if (priv_data.socket) {
		...
		result = sap_socket_send_data(priv_data.socket, HELLO_ACC_CHANNELID, strlen(message), message);
	} else {
			     ...
			     void *user_data)
{
	...
	update_ui(buffer);
	//char sentence_4[4] = {"Pggg"};
	//char sentence_5[4] = {"stop"};
	//buffer = sentence_4;

	//sap_socket_send_data(priv_data.socket, HELLO_ACC_CHANNELID, 4, buffer);
